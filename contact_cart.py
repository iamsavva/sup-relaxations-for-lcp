...
class ContactCart:
    def __init__(self, N=10, h=0.1, m=1, x0=1, v0=-1, xt=2, vt=0, Qx=1, Qv=0.01, Ru=0.01, Rf=0.0, umax=10, verbose=True, qcqp_scaling_factor = 0.01) -> None:
        assert x0 >= 0
        self.N = N
        self.h = h
        self.m = m
    ...
    def get_constraint_violation(self, x:npt.NDArray, v:npt.NDArray, f:npt.NDArray, u:npt.NDArray):
        lcp = x[1:] * f
        pos = abs(x[1:] - x[:-1] - self.h * v[1:])
        vel = abs(v[1:] - v[:-1] - self.h/self.m * (u+f) )
        return lcp+pos+vel
    
    def solve_with_ipopt(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
    ...
    def warmstart_with_ipopt(self, x_w, v_w, f_w, u_w)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        prog,x,v,f,u = self.form_nonlinear_prog()
    ...
    def warmstart_with_snopt(self, x_w, v_w, f_w, u_w)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        prog,x,v,f,u = self.form_nonlinear_prog()
    ...
    def solve_with_snopt(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        prog,x,v,f,u = self.form_nonlinear_prog()
        timer = timeit()
        solver = SnoptSolver()
        solution = solver.Solve(prog) # type: MathematicalProgramResult
    ...
    def solve_nonconvex_qcqp_sdp_relaxation(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        ...
        solve_time = timer.dt("nonconvex QCQP relaxaion with MOSEK",verbose=self.verbose)
        diditwork(solution,verbose=self.verbose)
        INFO("-------------------",verbose=self.verbose)
        cost = solution.get_optimal_cost()
        ev = np.vectorize(lambda a: a.Evaluate())
    ...
    def solve_chordal_nonconvex_qcqp_sdp_relaxation(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        ...
        for n in range(self.N):
            ...
            if n > 0:
                ...
                _, vu1, vu2 = extract_moments_from_spectrahedron_prog_variables(e.xu(), 6)
                _, vv1, vv2 = extract_moments_from_spectrahedron_prog_variables(e.xv(), 6)
            ...
            cost = Expression(0)
            if n == 0:
                cost += m2[0,0]*self.Qx + m0 * self.Qx * self.xt **2 - 2 * self.Qx * self.xt * m1[0]
                # print(m0 * self.Qx * self.xt **2)
                # print(2 * self.Qx * self.xt * m1[0])
                cost += m2[1,1]*self.Qv + m0 * self.Qv * self.vt **2 - 2 * self.Qv * self.vt * m1[1]
            
            cost += m2[2,2]*self.Rf
            cost += m2[3,3]*self.Ru
            cost += m2[4,4]*self.Qx + m0 * self.Qx * self.xt **2 - 2 * self.Qx * self.xt * m1[4]
            cost += m2[5,5]*self.Qv + m0 * self.Qv * self.vt **2 - 2 * self.Qv * self.vt * m1[5]
            v.AddCost(cost)
        ...
        solve_time = timer.dt("chordal noconvex-qcqp SDP relaxation",verbose=self.verbose)
        diditwork(solution,verbose=self.verbose)
        INFO("-------------------",verbose=self.verbose)
        ...
        for edge in edge_traj:
            m0, m1, m2 = extract_moments_from_spectrahedron_prog_variables(solution.GetSolution(edge.u().x()), 6)
            vertex_traj.append(m1[0:4])
        m0, m1, m2 = extract_moments_from_spectrahedron_prog_variables(solution.GetSolution(edge_traj[-1].v().x()), 6)
    ...
    def build_gcs(self)-> T.Tuple[GraphOfConvexSets, GraphOfConvexSets.Vertex, GraphOfConvexSets.Vertex]:
        ...
        for n in range(1, self.N):
            ...
            vc_n = gcs.AddVertex(HPolyhedron.MakeBox([0, -vmax, -umax, 0],[xmax, vmax, umax, fmax]), "vc_"+str(n))
        ...
        for k in range(0, self.N):
            n = str(k)
            n1 = str(k+1)
            ec = gcs.AddEdge(vertices["v_"+n], vertices["v_"+n1], "c v_"+n+"->v_"+n1)
            add_cons_in_contact(ec)
            en = gcs.AddEdge(vertices["v_"+n], vertices["v_"+n1], "n v_"+n+"->v_"+n1)
    ...
    def solve_micp_with_gcs(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        gcs, v0, vN = self.build_gcs()
        options = GraphOfConvexSetsOptions()
        ...
        return solve_time, solution.get_optimal_cost(), x_traj, v_traj, u_traj, f_traj, lcp_v_traj

    def solve_bad_qcqp_convex_diff_decomposition(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        ...
        for n in range(self.N):
            ...
            adjusted_cost += add_socp_constraint(prog, x[n+1], f[n], self.qcqp_scaling_factor, n)
        ...
        diditwork(solution,verbose=self.verbose)
        WARN(solution.get_optimal_cost() - solution.GetSolution(adjusted_cost),verbose=self.verbose)
        INFO("-------------------",verbose=self.verbose)

        cost = solution.get_optimal_cost() - solution.GetSolution(adjusted_cost).Evaluate()
        ev = np.vectorize(lambda a: a.Evaluate())
        x_traj = ev(solution.GetSolution(x).reshape(self.N+1))
        v_traj = ev(solution.GetSolution(v).reshape(self.N+1))
    ...
    def solve_bad_sdp_convex_diff_decomposition(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        def add_socp_constraint(prog:MathematicalProgram, x:npt.NDArray, y:npt.NDArray, cost:float, n):
            # add SDP relaxation of constraint x^T y = 0
    ...
    def solve_our_qcqp_convex_diff_decomposition(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        def add_socp_constraint(prog:MathematicalProgram, x:npt.NDArray, y:npt.NDArray):
    ...
    def solve_our_qcqp_convex_diff_decomposition_with_warmstart(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        solve_time, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj = self.solve_our_qcqp_convex_diff_decomposition()
        solve_time2, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj = self.warmstart_with_snopt( x_traj, v_traj, f_traj, u_traj )
        return solve_time+solve_time2, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj
    
    def solve_bad_qcqp_convex_diff_decomposition_with_warmstart(self)->T.Tuple[float, float, npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray,npt.NDArray]:
        solve_time, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj = self.solve_bad_qcqp_convex_diff_decomposition()
        solve_time2, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj = self.warmstart_with_snopt( x_traj, v_traj, f_traj, u_traj )
        return solve_time+solve_time2, cost, x_traj, v_traj, f_traj, u_traj, lcp_v_traj
    

def run_a_test_for_suboptimality_gap(Qx=1,Rf=0.1, qcqp_scaling_factor= 0.1):
    sdp_violations = []
    qcqp_violations = []
    sdp_times = []
    qcqp_times = []
    for v0 in range(-15, 15):
        cart = ContactCart(v0=v0, qcqp_scaling_factor = qcqp_scaling_factor, verbose = False, Qx =Qx, Rf=Rf, umax=50)

        chordal_time, chordal_sdp_cost, _, _, _, _, _ = cart.solve_chordal_nonconvex_qcqp_sdp_relaxation()
    ...
    print("their QCQP SNOPT", np.min(qcqp_violations), np.max(qcqp_violations), np.mean(qcqp_violations), np.mean(qcqp_times))
